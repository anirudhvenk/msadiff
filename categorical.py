...
class CategoricalFlow(nn.Module, ABC):
    ...
    def forward(self, t, pt, *cond_args):
        ...
        if t.dim() == 0:
            t = t.unsqueeze(0)
        enc_input = pt.view(-1, pt.size(1), self.n_class)
        vf = self.encoder(enc_input, t, *cond_args).view(-1, pt.size(1), self.n_class)
    ...
    def sample_euler(self, n_sample, n_steps, device, *cond_args, return_traj=False):
        ...
        p0 = self.preprocess(self.sample_prior(n_sample, cond_args[0].size(1), self.n_class, device=device))
        ps = [p0]
        p = p0
    ...
    def compute_nll_ode(self, p1, n_steps=200, tmax=1., tmin=0., exact=False, verbose=False):
        ...
        init_div = torch.zeros(p1.size(0), p1.size(1), 1, device=device, dtype=torch.float)
