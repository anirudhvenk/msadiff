...
class VAERunner():
    def __init__(self, config):
        ...
        if dist.get_rank() == 0:
            wandb.init(project="msadiff", name="standard_val", config=config.to_dict())
        ...
        if dist.get_rank() == 0:
            ...
            print("Permuter params: ", sum(p.numel() for p in permuter.parameters() if p.requires_grad))
        ...
        self.test_dataset = MSADataset(config, test=True)
    ...
    def collate_fn(self, data):
        ...
        mask = torch.zeros(seq_tokens.size(0), self.config.data.max_sequence_len, dtype=torch.bool).to(self.config.device)
    ...
    def train(self):
        for e in range(self.config.training.epochs):
            ...
            for single_seq_embeddings, pairwise_seq_embeddings, msa_tokens, mask in tqdm(self.test_loader):
                with torch.no_grad():
                    pred_msa, perm, mu, logvar = self.model(single_seq_embeddings, pairwise_seq_embeddings, msa_tokens.unsqueeze(-1).float(), mask)
                    loss_dict = self.loss(msa_tokens, pred_msa, mask, perm, mu, logvar)
                    
                val_recon_loss += loss_dict["recon_loss"].detach()
                val_ppl += loss_dict["ppl"].detach()
            ...
            if dist.get_rank() == 0:
                print({
                    ...
                    "val_recon_loss": val_recon_loss.item()/len(self.test_loader),
                    "val_ppl": val_ppl.item()/len(self.test_loader)
                })
                wandb.log({
                    "epoch": e,
                    "total_loss": total_loss.item()/len(self.train_loader),
